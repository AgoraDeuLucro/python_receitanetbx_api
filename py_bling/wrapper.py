...
class auth():
    ...
    def request(self, method="GET", url="", headers=None, params=None, data=None):
        ...
        # Usar formato Bearer para OAuth tokens (JWT exige enable-jwt em toda requisição autenticada)
        if self.access_token != "" and self.access_token != None:
            req_headers.update(self._JWT_HEADER)
...
class nfe(auth):
    ...
    def listar(self, date_start, date_end, tipo=1, all_pages=True, limite=100, **kwargs):
        ...
        # Aceita datetime, date ou string YYYY-MM-DD / YYYY-MM-DD HH:MM:SS
        def fmt(d, default_time="00:00:00"):
            if isinstance(d, datetime):
                return d.strftime("%Y-%m-%d %H:%M:%S")
            if isinstance(d, date):
                return f"{d.strftime('%Y-%m-%d')} {default_time}"
            s = str(d)
            if len(s) == 10:  # apenas YYYY-MM-DD
        ...
        while True:
            params = {
                ...
                "limite": limite,
                "tipo": tipo,
                "dataEmissaoInicial": fmt(date_start, "00:00:00"),
                "dataEmissaoFinal": fmt(date_end, "23:59:59"),
            }
